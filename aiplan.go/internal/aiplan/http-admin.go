...
func (s *Services) AddAdminServices(g *echo.Group) {
	...
	tarifficationGroup := staffPermissionGroup.Group("tariffication/")
	...
	tarifficationGroup.GET("", s.getAllTariffList)
	tarifficationGroup.POST("", s.createUserTariff)
	tarifficationGroup.GET(":userId/", s.getUserTariff)
	tarifficationGroup.POST(":userId/", s.updateUserTariff)
	tarifficationGroup.DELETE(":userId/", s.deleteUserTariff)
...
// @Accept json
// @Produce json
// @Param order_by query string false "Поле для сортировки (по умолчанию user_id)"
// @Param offset query int false "Смещение (пагинация) (-1 для начала)"
// @Param limit query int false "Лимит записей (максимум 1000, по умолчанию 100)"
// @Param desc query bool false "Сортировка по убыванию (по умолчанию true)"
// @Param search_query query string false "Строка поиска (по email, имени или фамилии)"
// @Success 200 {object} dao.PaginationResponse{result=[]dto.Tariffication} "Список тарифов успешно получен"
// @Failure 400 {object} apierrors.DefinedError "Ошибка в параметрах запроса"
// @Failure 403 {object} apierrors.DefinedError "Ошибка: доступ запрещен"
// @Failure 500 {object} apierrors.DefinedError "Внутренняя ошибка сервера"
// @Router /api/auth/admin/tariffication/ [get]
func (s *Services) getAllTariffList(c echo.Context) error {
	orderByParam := "user_id"
	offset := -1
	limit := 100
	desc := true
	searchQuery := ""
	...
	query := s.db.
		...
		Order(clause.OrderByColumn{
			Column: clause.Column{Table: dao.Tariffication{}.TableName(), Name: orderByParam},
			Desc:   desc,
		})
	if searchQuery != "" {
		query = query.Where("email = ? or first_name = ? or last_name = ?", "%"+searchQuery+"%", "%"+searchQuery+"%", "%"+searchQuery+"%")
	}

	var tariffs []dao.Tariffication
	resp, err := dao.PaginationRequest(
		offset,
		limit,
		query,
	...
	resp.Result = utils.SliceToSlice(resp.Result.(*[]dao.Tariffication), func(t *dao.Tariffication) dto.Tariffication { return *t.ToDTO() })

	return c.JSON(http.StatusOK, resp)
}

// createUserTariff godoc
// @id createUserTariff
...
// @Accept json
// @Produce json
// @Param tariff body dto.Tariffication true "Данные нового тарифа"
// @Success 200 {object} dto.Tariffication "Тариф успешно создан"
// @Failure 400 {object} apierrors.DefinedError "Ошибка в параметрах запроса"
// @Failure 403 {object} apierrors.DefinedError "Ошибка: доступ запрещен"
// @Failure 409 {object} apierrors.DefinedError "Ошибка: тариф уже существует"
// @Failure 500 {object} apierrors.DefinedError "Внутренняя ошибка сервера"
// @Router /api/auth/admin/tariffication/ [post]
func (s *Services) createUserTariff(c echo.Context) error {
	user := c.(AuthContext).User

	var req dao.Tariffication
	if err := c.Bind(&req); err != nil {
		return EError(c, err)
	}
	...
	if err := s.db.Omit(clause.Associations).Create(&req).Error; err != nil {
		if err == gorm.ErrDuplicatedKey {
			return EErrorDefined(c, apierrors.ErrTariffExist)
		}
		return EError(c, err)
	}
	return c.JSON(http.StatusOK, req.ToDTO())
}
...
// @Produce json
// @Param userId path string true "ID пользователя"
// @Success 200 {object} dto.Tariffication "Тариф успешно найден"
// @Failure 404 {object} apierrors.DefinedError "Ошибка: тариф не найден"
// @Failure 403 {object} apierrors.DefinedError "Ошибка: доступ запрещен"
// @Failure 500 {object} apierrors.DefinedError "Внутренняя ошибка сервера"
// @Router /api/auth/admin/tariffication/{userId} [get]
func (s *Services) getUserTariff(c echo.Context) error {
	userId := c.Param("userId")

	var tariff dao.Tariffication
	if err := s.db.Joins("User").Where("user_id = ?", userId).First(&tariff).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return EErrorDefined(c, apierrors.ErrTariffNotFound)
		}
		return EError(c, err)
	}
	return c.JSON(http.StatusOK, tariff.ToDTO())
}

// updateUserTariff godoc
// @id updateUserTariff
...
// @Produce json
// @Param userId path string true "ID пользователя"
// @Param tariff body dto.Tariffication true "Данные для обновления тарифа"
// @Success 200 {object} dto.Tariffication "Тариф успешно обновлён"
// @Failure 400 {object} apierrors.DefinedError "Ошибка в параметрах запроса"
// @Failure 403 {object} apierrors.DefinedError "Ошибка: доступ запрещен"
// @Failure 409 {object} apierrors.DefinedError "Ошибка: тариф уже существует"
// @Failure 500 {object} apierrors.DefinedError "Внутренняя ошибка сервера"
// @Router /api/auth/admin/tariffication/{userId} [put]
func (s *Services) updateUserTariff(c echo.Context) error {
	user := c.(AuthContext).User
	...
	if err := s.db.Omit("user_id", "created_by_id", "created_at", "user_detail").Updates(&req).Error; err != nil {
		if err == gorm.ErrDuplicatedKey {
			return EErrorDefined(c, apierrors.ErrTariffExist)
		}
		return EError(c, err)
...
// @Produce json
// @Param userId path string true "ID пользователя"
// @Success 200 "Тариф успешно удалён"
// @Failure 404 {object} apierrors.DefinedError "Ошибка: тариф не найден"
// @Failure 403 {object} apierrors.DefinedError "Ошибка: доступ запрещен"
// @Failure 500 {object} apierrors.DefinedError "Внутренняя ошибка сервера"
// @Router /api/auth/admin/tariffication/{userId} [delete]
func (s *Services) deleteUserTariff(c echo.Context) error {
	userId := c.Param("userId")

	if err := s.db.Where("user_id = ?", userId).Delete(&dao.Tariffication{}).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return EErrorDefined(c, apierrors.ErrTariffNotFound)
		}
		return EError(c, err)
