// Package rules реализует движок пользовательских Lua-скриптов для автоматизации
// workflow задач (Issues) в проектах.
//
// Администраторы проекта могут написать Lua-скрипт, который будет вызываться
// при изменении задачи. Скрипт может разрешить или запретить действие,
// реализуя таким образом кастомные бизнес-правила (например, "только PM может
// переводить задачу в статус Done" или "нельзя убирать всех assignees").
//
// Поддерживаемые хуки (функции, которые может определить пользователь в скрипте):
//   - BeforeStatusChange(ctx, newState) — перед сменой статуса задачи
//   - AfterStatusChange(ctx, newState) — после смены статуса задачи
...
func callEventFunction(fnName string, state *lua.LState, issuer dao.User, currentIssue dao.Issue, params ...interface{}) (LuaResp, []LuaOut, IRulesError) {
	...
	info := &debugInfo{
		Function:    &fnName,
		ProjectId:   currentIssue.ProjectId.String(),
