// Package filestorage абстрагирует хранилище файлов (вложения, аватары, экспорты).
//
// Реализации:
//   - MinioStorage — production-хранилище на базе MinIO/S3-совместимого API
//   - LocalStorage — локальная файловая система (для разработки)
//
// Поддерживает:
//   - Сохранение/загрузку файлов по UUID
//   - TUS-протокол для resumable uploads больших файлов
//   - Метаданные (workspaceId, projectId, issueId) для аудита и очистки
//   - Retry-логику при временных ошибках MinIO (до 20 попыток)
//
// UUID-именование файлов обеспечивает уникальность и скрывает оригинальные имена.
package filestorage
