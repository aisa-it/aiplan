// Package integrations реализует интеграции с внешними системами контроля
// версий (GitHub, GitLab) через входящие Webhooks.
//
// При получении события от GitHub/GitLab (push, merge request, issue)
// интеграция создаёт комментарий в связанной задаче AIPlan или обновляет
// её статус. Связь определяется по упоминанию идентификатора задачи
// (например, "PROJ-123") в commit message или описании MR/PR.
//
// Каждая интеграция представлена как системный пользователь (bot),
// от имени которого создаются комментарии. Интеграцию можно подключить
// к рабочему пространству через настройки.
