...
func formatByField(
	act *dao.ActivityEvent,
	m map[actField.ActivityField]funMsgFormat,
	defaultFn func(*dao.ActivityEvent, actField.ActivityField) TgMsg,
) (TgMsg, error) {
	var res TgMsg

	if act.Field == "" {
		return res, fmt.Errorf("%s field is nil", act.EntityType.String())
	}
	...
	if res.IsEmpty() {
		return res, fmt.Errorf("%s %w, verb: %s, field: %s, id: %s", act.EntityType.String(), ErrEmptyActivity, act.Verb, act.Field, act.ID)
	}
