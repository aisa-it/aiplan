...
func (ia *issueActivity) getMails(tx *gorm.DB) []mail {
	...
	for _, member := range ia.users {
		...
		for _, activity := range ia.activities {
			...
			memberNotify = member.ProjectMemberSettings.IsNotify(actField.ActivityField(*activity.Field), types.LayerIssue, activity.Verb, member.ProjectRole)
			if activity.Issue.CreatedById == member.User.ID {
				authorNotify = member.ProjectAuthorSettings.IsNotify(actField.ActivityField(*activity.Field), types.LayerIssue, activity.Verb, member.ProjectRole)
			}
