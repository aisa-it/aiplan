...
func (da *docActivity) getMails(tx *gorm.DB) []mail {
	...
	for _, member := range da.users {
		...
		for _, activity := range da.activities {
			...
			memberNotify = member.DocMemberSettings.IsNotify(actField.ActivityField(*activity.Field), types.LayerDoc, activity.Verb, member.WorkspaceRole)
			if activity.Doc.CreatedById == member.User.ID {
				authorNotify = member.DocAuthorSettings.IsNotify(actField.ActivityField(*activity.Field), types.LayerDoc, activity.Verb, member.WorkspaceRole)
			}
