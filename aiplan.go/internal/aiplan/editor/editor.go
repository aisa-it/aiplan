// Package editor парсит HTML из TipTap редактора в структурированное
// представление Document для последующего экспорта (PDF, plain text).
//
// TipTap — WYSIWYG редактор на базе ProseMirror, используемый в AIPlan
// для описания задач и документов. Пакет понимает как стандартные HTML-теги,
// так и кастомные data-атрибуты TipTap:
//   - data-spoiler, data-collapsed — сворачиваемые блоки
//   - data-info-block, data-icon-color — информационные блоки
//   - data-type="taskList" — чек-листы
//
// Также поддерживается парсинг из TipTap JSON (через подпакет tiptap),
// что предпочтительнее, т.к. сохраняет типизацию элементов.
package editor
