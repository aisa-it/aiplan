...
func (s *Services) WorkspaceMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		*/
		...
		if err := s.DB(c).Where("workspace_id = ?", workspace.ID).Where("member_id = ?", user.ID).First(&workspaceMember).Error; err != nil {
			if err == gorm.ErrRecordNotFound {
				return EErrorDefined(c, apierrors.ErrWorkspaceNotFound)
			}
			return EError(c, err)
...
func (s *Services) updateWorkspaceMember(c echo.Context) error {
	...
	if req.Role != nil {
		...
		// роли вызывающего (проверка на :697 сравнивает лишь с текущей ролью цели).
...
func (s *Services) addIntegrationToWorkspace(c echo.Context) error {
	...
	if err := s.DB(c).Save(&workspaceMember).Error; err != nil {
		return EError(c, err)
	}
