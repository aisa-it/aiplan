// Package apierrors определяет типизированные ошибки API с поддержкой
// локализации (английский + русский) и HTTP-статусов.
//
// Каждая ошибка имеет:
//   - Code — уникальный числовой код для идентификации на клиенте
//   - StatusCode — HTTP-статус для ответа (400, 401, 403, 404, 409, ...)
//   - Err — сообщение на английском
//   - RuErr — сообщение на русском
//
// Коды сгруппированы по доменам:
//   - 1xxx — аутентификация и сессии
//   - 2xxx — рабочие пространства
//   - 3xxx — проекты, формы, документы, спринты
//   - 4xxx — задачи (issues)
//   - 5xxx — валидация
//   - 6xxx — пользователи
//   - 7xxx — интеграции
//   - 8xxx — импорт
//   - 9xxx — администрирование
//   - 10xxx — Git
//   - 11xxx — SSH
package apierrors
...
var (
	...
	ErrResetPasswordForbidden   = DefinedError{Code: 6017, StatusCode: http.StatusForbidden, Err: "pasword resets forbidden in current enveronment", RuErr: "Сброс пароля недоступен в данном окружении"}
