...
// ReadConfig читает конфигурацию и завершает процесс при ошибке.
// Для библиотечного использования (встраивание ядра в другой модуль)
// применять LoadConfig, возвращающий ошибку.
...
// ErrWebURLRequired — не задан обязательный параметр WEB_URL.
var ErrWebURLRequired = errors.New("WEB_URL is required")

// LoadConfig читает конфигурацию из JSON-файла (если путь задан) и переменных
// окружения; env перекрывает значения из файла.
...
// applyDefaults подставляет значения для незаданных или некорректных параметров.
func (c *Config) applyDefaults() {
	if c.NotificationsSleep <= 0 || c.NotificationsSleep > 59 {
		c.NotificationsSleep = 5
	}
	...
	if c.LDAPServerURL.URL != nil && c.LDAPFilter == "" {
		c.LDAPFilter = "(&(uniqueIdentifier={email}))"
	}

	if c.OTELSampleRate == 0 {
		c.OTELSampleRate = 0.1
	}

	if c.CaptchaSecret == "" && !c.CaptchaDisabled {
		c.CaptchaSecret = uuid.Must(uuid.NewV4()).String()
	}
}
