...
// SetConfig задаёт конфигурацию пакета. Вызывать до первого обращения к БД:
// пакетные переменные — переходное решение, повторная установка в рамках
// процесса не поддерживается.
func SetConfig(c *config.Config) {
	Config = c
}

// SetFileStorage задаёт хранилище файлов. Условия те же, что у SetConfig.
func SetFileStorage(fs filestorage.FileStorage) {
	FileStorage = fs
}
