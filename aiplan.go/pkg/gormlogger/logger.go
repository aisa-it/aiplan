// Package gormlogger реализует кастомный логгер для GORM,
// интегрированный со стандартным slog.
//
// Особенности:
//   - Логирует SQL-запросы с временем выполнения и количеством затронутых строк
//   - Выделяет медленные запросы (превышающие SlowThreshold) уровнем Warn
//   - Логирует ошибки БД (кроме ErrRecordNotFound) уровнем Error
//   - Опционально скрывает параметры запросов (ParameterizedQueries)
//     для безопасности в production-логах
//   - DELETE-запросы исключены из slow query логов (они часто медленные by design)
package gormlogger
