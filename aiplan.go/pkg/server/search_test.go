...
func TestMain(m *testing.M) {
	// ReadConfig завершает процесс без WEB_URL — подставляем заглушку,
	// чтобы тесты без БД и окружения (например, проверки прав) запускались
	if os.Getenv("WEB_URL") == "" {
		os.Setenv("WEB_URL", "http://localhost:8080")
	}
